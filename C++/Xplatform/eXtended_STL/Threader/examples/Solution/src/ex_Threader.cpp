...
template<typename T>
void test_exception(xstring input, Threader<T>& nex_ull) {
    try {
...
template<typename T>
void test_exception(size_t input, Threader<T>& nex_ull) {
    try {
        cout << nex_ull.get(input) << endl;
    }
    catch (const std::exception & exc) {
...
void loop_str_prime_func(Threader<xstring>& nex) {
