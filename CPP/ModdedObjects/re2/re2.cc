...
static const std::string* empty_string = nullptr;
static const std::map<std::string, int>* empty_named_groups = nullptr;
static const std::map<int, std::string>* empty_group_names = nullptr;
...
{
  static std::atomic<bool> LbCalled = false;
  if (!LbCalled.exchange(true))
  {
      if (empty_string) delete empty_string;
      if (empty_named_groups) delete empty_named_groups;
      if (empty_group_names) delete empty_group_names;
  ...
  if (rprog_) delete rprog_;
  if (named_groups_) delete named_groups_;
  if (group_names_) delete group_names_;
  ...
  if (entire_regexp_ == NULL) {
    ...
    if (error_arg_) delete error_arg_;
