...
class RE2 {
 ...
 private:
  ...
  const std::string* pattern_ = nullptr;    // string regular expression
  Options options_;                         // option flags
  re2::Regexp* entire_regexp_ = nullptr;    // parsed regular expression
  re2::Regexp* suffix_regexp_ = nullptr;    // parsed regular expression, prefix_ removed
  const std::string* error_ = nullptr;      // error indicator (or points to empty string)
  const std::string* error_arg_ = nullptr;  // fragment of regexp showing error (or ditto)
  ...
  re2::Prog* prog_ = nullptr;               // compiled program for regexp

  // Reverse Prog for DFA execution only
  mutable re2::Prog* rprog_ = nullptr;
  // Map from capture names to indices
  mutable const std::map<std::string, int>* named_groups_ = nullptr;
  // Map from capture indices to names
  mutable const std::map<int, std::string>* group_names_ = nullptr;
